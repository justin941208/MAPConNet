...
class SMAL_DATA(data.Dataset):
    def __init__(self, opt, train, labelled=True):
        ...
        if self.train:
            if self.labelled:
                datapath_file = None if opt.percentage == 0 else f'animal_datapath_labelled_{opt.percentage}%.txt'
            else:
                datapath_file = None if opt.percentage == 100 else f'animal_datapath_unlabelled_{100 - opt.percentage}%.txt'

            if datapath_file is not None:
                datapath_file = os.path.join('data', 'datalists', datapath_file)
                if not os.path.exists(datapath_file):
                    data_list(opt.percentage, self.num_total, 'animal')
            ...
            print(f'Training set. Animal. Labelled: {self.labelled}. Size: {len(self.datapath)}. #IDs: {self.num_id}. #Poses: {self.num_pose}.')
        else:
            self.lines = [line for line in open(os.path.join('data', 'datalists', 'animal_test_list'), "r")]
            print(f'Test set. Animal. Size: {len(self.lines)}.')
    ...
    def __getitem__(self, index):
        if self.train:
            ...
            pose_mesh_i = np.random.choice(self.ids)
            pose_mesh_p = np.random.choice(self.poses)
            ...
            gt_mesh_name = 'toy_'+str(identity_mesh_i)+'_'+str(pose_mesh_p)+'.ply'
            if self.opt.use_unlabelled:
                ...
                pose_mesh2 = pymesh.load_mesh(self.path+'toy_'+str(pose_mesh_i)+'_'+str(pose_mesh2_p)+'.ply')
        else:
